...
import { queryFacts, getRecentFacts, getAllProjectFacts, getAllCompanies } from "./onedrive.js";
...
const SUMMARY_CACHE_TTL_S  = 28 * 60 * 60; // 28h — survives until next morning's cron
...
export async function handleSummary(env, chatId, project) {
  ...
  // Serve from KV cache only when the project has no new emails since last generation.
  // appendFacts() sets mem:dirty:{company} whenever new facts arrive; we must regenerate
  // in that case so same-day emails are not omitted from the briefing.
  const isDirty = await env.DAYA_KV.get(`mem:dirty:${company}`);
  const cached = await getCachedSummary(env, company);
  if (cached?.json && !isDirty) {
  ...
  // Cache miss or dirty — generate on-demand, then cache for the rest of the day
  const allFacts = await getAllProjectFacts(env, company);
  if (!allFacts.length) {
    return {
      text: `📊 <b>${escHtml(label)}</b>\n\nNo facts recorded yet for this project.`,
      json: null,
    };
  }
  ...
  if (!recent.length) {
    return {
      text: `📊 <b>${escHtml(label)}</b>\n\nNo activity in the last ${SUMMARY_DAYS_WINDOW} days.`,
      json: null,
    };
  }

  const json = await callClaudeForSummary(env, label, recent);
  await setCachedSummary(env, company, { json, generatedAt: new Date().toISOString() });
  // Clear the dirty flag now that the cache reflects the latest facts.
  await env.DAYA_KV.delete(`mem:dirty:${company}`);

  const text = formatSummaryText(label, json);
  await env.DAYA_KV.put(`tg:bot:draft:${chatId}`, JSON.stringify(json),
    { expirationTtl: 2 * 60 * 60 });
  return { text, json };
}
